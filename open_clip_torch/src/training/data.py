...
# def get_uid(stream, fname):
#     key, extension = fname.split('.') 
#     json_file = stream.extractfile(f'{key}.json').read()
#     uid = json.loads(json_file.decode())['uid']
#     return uid
...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    if args.subset_file == "":
        ...
        with open(args.subset_file, 'rb') as f:
            ...
            trie = pickle.load(f)
            if is_master(args):
                logging.info(f"The dataset will be pruned using the trie object stored at {args.subset_file}") 
    ###############################
    ...
    if is_train:
        ...
        #print(f'global batch size {global_batch_size}, number of samples {num_samples}, number of workers {num_workers}, number of batch {num_batches}')  
    else:
